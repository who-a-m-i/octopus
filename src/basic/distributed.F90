...
contains
  ...
  subroutine distributed_init(this, total, comm, message, tag, scalapack_compat)
    ...
    if(this%mpi_grp%size == 1 .or. this%nglobal == 1) then
      ...
      if(present(tag)) then
        this%message%lines(1) = 'Info: Parallelization in ' // trim(tag)
      ...
      do kk = 1, this%mpi_grp%size
        ...
        if(present(tag)) then
          write(this%message%lines(1),'(a,i4,a,i6,a)') 'Info: Node in group ', kk - 1, &
            ' will manage ', this%num(kk - 1), ' '//trim(tag)
          if(this%num(kk - 1) > 0) then
            write(this%message%lines(1),'(a,a,i6,a,i6)') trim(this%message%lines(1)), ':', &
