...
contains
  ...
  subroutine mpi_debug_statistics()
    ...
    messages(2) = hyphens
    messages(3) = ''
    write(messages(4), '(23x,a,4x,a,8x,a)') 'total time', 'calls', 'usec/call'
    do j = 1, C_NUM_MPI_ROUTINES
      ...
      write(messages(j+4),'(a,f13.6,6x,i4,6x,f13.0)') &
        mpi_rlabel(j)//' : ', sec_accum(j),          &
        call_counter(j), usec_call(j)
    end do
    messages(C_NUM_MPI_ROUTINES+5) = ''
    messages(C_NUM_MPI_ROUTINES+6) = hyphens
  ...
  subroutine mpi_debug_in(comm, index)
    ...
    sec_in              = MPI_Wtime()
    write(messages(1),'(a,f18.6,a,z8.8,a,i6.6,a,f13.6)') '* MPI_I ', &
  ...
  subroutine mpi_debug_out(comm, index)
    ...
    write(messages(1),'(a,f18.6,a,z8.8,a,i6.6,a,f13.6,a,f13.6)')         &
