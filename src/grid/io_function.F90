...
contains
  ...
  subroutine io_function_read_how(sb, namespace, how, ignore_error)
    ...
    if(how  ==  0 .and. .not. optional_default(ignore_error, .false.)) then
      write(messages(1), '(a)') 'Must specify output method with variable OutputFormat.'
    ...
    if(sb%dim == 1) then
      if(bitand(how, OPTION__OUTPUTFORMAT__AXIS_Y) /= 0) then
        messages(1) = "OutputFormat = axis_y not available with Dimensions = 1."
        call message_g%fatal(1)
      end if
      if(bitand(how, OPTION__OUTPUTFORMAT__PLANE_Z) /= 0) then
        messages(1) = "OutputFormat = plane_z not available with Dimensions = 1."
        call message_g%fatal(1)
      end if
      if(bitand(how, OPTION__OUTPUTFORMAT__XCRYSDEN) /= 0) then
        messages(1) = "OutputFormat = xcrysden not available with Dimensions = 1."
        call message_g%fatal(1)
      end if
    end if
    ...
    if(sb%dim <= 2) then
      if(bitand(how, OPTION__OUTPUTFORMAT__AXIS_Z) /= 0) then
        messages(1) = "OutputFormat = axis_z not available with Dimensions <= 2."
        call message_g%fatal(1)
      end if
      if(bitand(how, OPTION__OUTPUTFORMAT__PLANE_X) /= 0) then
        messages(1) = "OutputFormat = plane_x not available with Dimensions <= 2."
        call message_g%fatal(1)
      end if
      if(bitand(how, OPTION__OUTPUTFORMAT__PLANE_Y) /= 0) then
        messages(1) = "OutputFormat = plane_y not available with Dimensions <= 2."
        call message_g%fatal(1)
      end if
      if(bitand(how, OPTION__OUTPUTFORMAT__INTEGRATE_XY) /= 0) then
        messages(1) = "OutputFormat = integrate_xy not available with Dimensions <= 2."
        call message_g%fatal(1)
      end if
      if(bitand(how, OPTION__OUTPUTFORMAT__INTEGRATE_XZ) /= 0) then
        messages(1) = "OutputFormat = integrate_xz not available with Dimensions <= 2."
        call message_g%fatal(1)
      end if
      if(bitand(how, OPTION__OUTPUTFORMAT__INTEGRATE_YZ) /= 0) then
        messages(1) = "OutputFormat = integrate_yz not available with Dimensions <= 2."
        call message_g%fatal(1)
      end if
      if(bitand(how, OPTION__OUTPUTFORMAT__DX) /= 0) then
        messages(1) = "OutputFormat = dx not available with Dimensions <= 2."
        call message_g%fatal(1)
      end if
      if(bitand(how, OPTION__OUTPUTFORMAT__CUBE) /= 0) then
        messages(1) = "OutputFormat = cube not available with Dimensions <= 2."
        call message_g%fatal(1)
      end if
    end if
    ...
    if (bitand(how, OPTION__OUTPUTFORMAT__NETCDF) /= 0) then
      messages(1) = 'Octopus was compiled without NetCDF support.'
      messages(2) = 'It is not possible to write output in NetCDF format.'
      call message_g%fatal(2)
    end if
#endif
#if !defined(HAVE_ETSF_IO)
    if (bitand(how, OPTION__OUTPUTFORMAT__ETSF) /= 0) then
      messages(1) = 'Octopus was compiled without ETSF_IO support.'
      messages(2) = 'It is not possible to write output in ETSF format.'
      call message_g%fatal(2)
    end if
#endif
  ...
  subroutine ncdf_error(func, status, filename, ierr)
    ...
    write(messages(1),'(3a)') "NETCDF error in function '" , trim(func) , "'"
    write(messages(2),'(3a)') "(reading/writing ", trim(filename) , ")"
    write(messages(3), '(6x,a,a)')'Error code = ', trim(nf90_strerror(status))
