...
subroutine X(io_function_input_global)(filename, namespace, mesh, ff, ierr, map)
  ...
  case("csv")
    if (mesh%sb%box_shape /= PARALLELEPIPED .and. mesh%sb%box_shape /= HYPERCUBE) then
      messages(1) = "Box shape must be parallelepiped or hypercube when a .csv file is used."
    ...
    if (ierr /= 0) then
      messages(1) = "Could not read file "//trim(io_workpath(filename, namespace))//""
      call message_g%fatal(1)
    end if
...
! Define the format
  ...
  if(bitand(how, OPTION__OUTPUTFORMAT__BOUNDARY_POINTS) /= 0) then
    if(ubound(ff, dim = 1) >= mesh%np_part_global) then
      ...
      write(messages(1),'(2a)') trim(fname), ': not outputting boundary points; they are not available'
...
! NOTE: MJV: how could this return anything but ix=0? Answ: if there is a shift in origin
  ...
  subroutine out_xcrysden(write_real)
    ...
    if(mesh%sb%dim /= 3 .and. mesh%sb%dim /= 2) then
      write(messages(1), '(a)') 'Cannot output function '//trim(fname_ext)//' in XCrySDen format except in 2D or 3D.'
      call message_g%warning(1)
      return
    end if
