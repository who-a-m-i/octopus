...
subroutine X(io_function_input)(filename, namespace, mesh, ff, ierr, map)
  ...
  if(mesh%parallel_in_domains) then
    ...
    if(debug%info) call message_g%debug_newlines(2)
...
subroutine X(io_function_input_global)(filename, namespace, mesh, ff, ierr, map)
  ...
  case("ncdf")
    ...
    if(status /= NF90_NOERR) then
      ...
      call cube_init(cube, mesh%idx%ll, mesh%sb, namespace, message_g)
  ...
  case("csv")
    if (mesh%sb%box_shape /= PARALLELEPIPED .and. mesh%sb%box_shape /= HYPERCUBE) then
      message_g%lines(1) = "Box shape must be parallelepiped or hypercube when a .csv file is used."
    ...
    if (ierr /= 0) then
      message_g%lines(1) = "Could not read file "//trim(io_workpath(filename, namespace))//""
      call message_g%fatal(1)
    end if
...
#else
    call message_g%not_implemented("Reading complex CSV file")
#endif
  ...
  if(mesh%parallel_in_domains) then
    ...
    if (.not. is_global_) then
      if(bitand(how, OPTION__OUTPUTFORMAT__BOUNDARY_POINTS) /= 0) then
        call message_g%not_implemented("OutputFormat = boundary_points with domain parallelization")
...
! Define the format
  ...
  if(bitand(how, OPTION__OUTPUTFORMAT__BOUNDARY_POINTS) /= 0) then
    if(ubound(ff, dim = 1) >= mesh%np_part_global) then
      ...
      write(message_g%lines(1),'(2a)') trim(fname), ': not outputting boundary points; they are not available'
...
! NOTE: MJV: how could this return anything but ix=0? Answ: if there is a shift in origin
  ...
  subroutine out_xcrysden(write_real)
    ...
    if(mesh%sb%dim /= 3 .and. mesh%sb%dim /= 2) then
      write(message_g%lines(1), '(a)') 'Cannot output function '//trim(fname_ext)//' in XCrySDen format except in 2D or 3D.'
      call message_g%warning(1)
      return
    end if
  ...
  subroutine out_vtk()
    ...
    call cube_init(cube, mesh%idx%ll, mesh%sb, namespace, message_g, spacing = dk )
    call cube_function_null(cf)
    call X(cube_function_alloc_RS) (cube, cf)
    call X(mesh_to_cube) (mesh, ff, cube, cf)
...
! Define the format
  ...
  if(bitand(how, OPTION__OUTPUTFORMAT__BINARY)     /= 0) call message_g%not_implemented("Outpur_KPT with format binary") 
  if(bitand(how, OPTION__OUTPUTFORMAT__AXIS_X)     /= 0) call message_g%not_implemented("Outpur_KPT with format axis x")
  if(bitand(how, OPTION__OUTPUTFORMAT__AXIS_Y)     /= 0) call message_g%not_implemented("Outpur_KPT with format axis y")
  if(bitand(how, OPTION__OUTPUTFORMAT__AXIS_Z)     /= 0) call message_g%not_implemented("Outpur_KPT with format axis z")
  if(bitand(how, OPTION__OUTPUTFORMAT__PLANE_X)    /= 0) call out_plane(1, 2, 3) ! x=0; y; z;
  if(bitand(how, OPTION__OUTPUTFORMAT__PLANE_Y)    /= 0) call out_plane(2, 1, 3) ! y=0; x; z;
  if(bitand(how, OPTION__OUTPUTFORMAT__PLANE_Z)    /= 0) call out_plane(3, 1, 2) ! z=0; x; y;
  if(bitand(how, OPTION__OUTPUTFORMAT__DX)         /= 0) call message_g%not_implemented("Outpur_KPT with format dx")
  if(bitand(how, OPTION__OUTPUTFORMAT__XCRYSDEN)   /= 0) call message_g%not_implemented("Outpur_KPT with format xcrysden")
  if(bitand(how, OPTION__OUTPUTFORMAT__CUBE)       /= 0) call message_g%not_implemented("Outpur_KPT with format cube")

  if(bitand(how, OPTION__OUTPUTFORMAT__MATLAB) /= 0) call message_g%not_implemented("Outpur_KPT with format matlab")

#if defined(HAVE_NETCDF)
  if(bitand(how, OPTION__OUTPUTFORMAT__NETCDF)     /= 0) call message_g%not_implemented("Outpur_KPT with format netcdf")
#endif
  if(bitand(how, OPTION__OUTPUTFORMAT__VTK) /= 0) call message_g%not_implemented("Outpur_KPT with format vtk")
