...
subroutine X(restart_write_mesh_function)(restart, filename, mesh, ff, ierr, root)
  ...
  if (i_am_root) then
    ...
    if (ierr /= 0) then
      messages_lines(1) = "Unable to write restart function to '"//trim(restart%pwd)//"/"//trim(filename)//"'."
...
subroutine X(restart_read_mesh_function)(restart, filename, mesh, ff, ierr)
  ...
  if (ierr /= 0) then
    messages_lines(1) = "Unable to read mesh function from '"//&
...
subroutine X(restart_write_binary1)(restart, filename, np, ff, ierr, root)
  ...
  if (ierr /= 0) then
    messages_lines(1) = "Unable to write restart information to '"//trim(full_filename)//"'."
    call messages_warning(1)
  end if
...
subroutine X(restart_read_binary1)(restart, filename, np, ff, ierr)
  ...
  if (ierr /= 0) then
    messages_lines(1) = "Unable to read restart information from '"//trim(full_filename)//"'."
    call messages_warning(1)
  end if
