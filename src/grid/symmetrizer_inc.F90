  ...
  if(.not. optional_default(suppress_warning, .false.)) then
    if(present(field)) then
      ...
      if(maxabsdiff / maxabs > CNST(1e-6)) then
        write(message%lines(1),'(a, es12.5)') 'Symmetrization discrepancy ratio (scalar) = ', maxabsdiff / maxabs
    ...
    if(present(field_vector)) then
      ...
      if(maxabsdiff / maxabs > CNST(1e-6)) then
        write(message%lines(1),'(a, es12.5)') 'Symmetrization discrepancy ratio (vector) = ', maxabsdiff / maxabs
        call messages_warning(1)
      end if
    end if
  end if
