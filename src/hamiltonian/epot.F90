...
contains
  ...
  subroutine epot_init(ep, namespace, gr, geo, psolver, ispin, nik, xc_family)
    ...
    if(.not.varinfo_valid_option('FilterPotentials', filter)) call message_g%input_error('FilterPotentials')
    call message_g%print_var_option(stdout, "FilterPotentials", filter)

    if(family_is_mgga(xc_family) .and. filter /= PS_FILTER_NONE) &
      call message_g%not_implemented("FilterPotentials different from filter_none with MGGA")
    ...
    if(geo%ncatoms > 0) then
      ...
      if(simul_box_is_periodic(gr%mesh%sb)) &
        call message_g%not_implemented("classical atoms in periodic systems")
      ...
      if(ep%classical_pot  ==  CLASSICAL_GAUSSIAN) then
        call message_g%experimental("Gaussian smeared classical charges")
    ...
    call message_g%obsolete_variable(namespace, 'UserDefinedTDPotential', 'TDExternalFields')
    ...
    if(parse_block(namespace, 'StaticMagneticField', blk) == 0) then
      ...
      if(.not.varinfo_valid_option('StaticMagneticField2DGauge', gauge_2d)) &
        call message_g%input_error('StaticMagneticField2DGauge')
      ...
      if(gr%sb%dim > 3) call message_g%not_implemented('Magnetic field for dim > 3')
    ...
    call message_g%print_var_option(stdout, "RelativisticCorrection", ep%reltype)
    ...
    if(ep%ignore_external_ions) then
      if(gr%sb%periodic_dim > 0) call message_g%input_error('IgnoreExternalIons')
    end if
    ...
    if(ep%force_total_enforce) call message_g%experimental('ForceTotalEnforce')
    ...
    if(parse_is_defined(namespace, 'TDGlobalForce')) then
      ...
      if(ierr /= 0) then
        call message_g%write("You have enabled the GlobalForce option but Octopus could not find")
        call message_g%write("the '"//trim(function_name)//"' function in the TDFunctions block.")
  ...
  subroutine epot_generate_classical(ep, mesh, geo)
    ...
    do ia = 1, geo%ncatoms
      do ip = 1, mesh%np
        ...
        case default
          call message_g%input_error('ClassicalPotential')
        end select
      end do
    end do
