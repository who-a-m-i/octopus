...
contains
  ...
  subroutine gauge_field_init(this, namespace, sb)
    ...
    if(parse_block(namespace, 'GaugeVectorField', blk) == 0) then
      ...
      if(.not. simul_box_is_periodic(sb)) then
        messages(1) = "GaugeVectorField is intended for periodic systems."
      ...
      if(sb%kpoints%use_symmetries) then
        do iop = 1, symmetries_number(sb%symm)
          ...
          if(.not. symm_op_invariant_cart(sb%symm%ops(iop), this%vecpot_kick, CNST(1e-5))) then
            messages(1) = "The GaugeVectorField breaks (at least) one of the symmetries used to reduce the k-points."
            messages(2) = "Set SymmetryBreakDir equal to GaugeVectorField."
  ...
  subroutine gauge_field_propagate(this, dt, time)
    ...
    do idim = 1, this%ndim
      if(.not. warning_shown .and. this%vecpot_kick(idim) /= M_ZERO .and.  &
        ...
        write(messages(1),'(a)') 'It seems that the gauge-field might be diverging. You should probably check'
        write(messages(2),'(a)') 'the simulation parameters, in particular the number of k-points.'
  ...
  subroutine gauge_field_init_vec_pot(this, sb, st)
    ...
    write (messages(1), '(a,f12.6,a)') "Info: Electron-gas plasmon frequency", &
  ...
  subroutine gauge_field_dump(restart, gfield, ierr)
    ...
    if (debug%info) then
      messages(1) = "Debug: Writing gauge field restart."
      call message_g%info(1)
    end if
    ...
    if (debug%info) then
      messages(1) = "Debug: Writing gauge field restart done."
      call message_g%info(1)
    end if
  ...
  subroutine gauge_field_load(restart, gfield, ierr)
    ...
    if (debug%info) then
      messages(1) = "Debug: Reading gauge field restart."
      call message_g%info(1)
    end if
    ...
    if (debug%info) then
      messages(1) = "Debug: Reading gauge field restart done."
      call message_g%info(1)
    end if
