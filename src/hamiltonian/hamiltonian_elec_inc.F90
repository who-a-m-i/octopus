...
subroutine X(hamiltonian_elec_apply) (hm, der, psolver, psi, hpsi, ist, ik, terms, set_bc, set_phase)
  ...
  call batch_init(psib, hm%d%dim, 1, message_g)
  call batch_add_state(psib, ist, psi)
  call batch_init(hpsib, hm%d%dim, 1, message_g)
...
subroutine X(exchange_operator)(hm, der, psolver, ik, exx_coef, psib, hpsib)
  ...
  if(der%mesh%sb%kpoints%full%npoints > 1) call message_g%not_implemented("exchange operator with k-points")
...
subroutine X(exchange_operator_hartree) (hm, der, psolver, ik, exx_coef, psib, hpsib)
  ...
  if(der%mesh%sb%kpoints%full%npoints > 1) call message_g%not_implemented("exchange operator with k-points")
  if(hm%hf_st%parallel_in_states) call message_g%not_implemented("exchange operator parallel in states")
...
subroutine X(magnus) (hm, der, psolver, psi, hpsi, ik, vmagnus, set_phase)
  ...
  if(hm%d%dim /= 1) &
    call message_g%not_implemented("Magnus with spinors")
