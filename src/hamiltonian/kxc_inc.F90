...
subroutine xc_get_kxc(xcs, mesh, rho, ispin, kxc)
  ...
  if(bitand(xcs%kernel_family, XC_FAMILY_LDA) == 0) then
    messages_lines(1) = "Only LDA functionals are authorized for now in XCKernel."
  ...
  do ixc = 1, 2
    if(bitand(functl(ixc)%flags, XC_FLAGS_HAVE_KXC) == 0) then
      messages_lines(1) = "Cannot calculate kernel derivative. This functional does not have Kxc available."
      call messages_fatal(1)
    end if
  end do
...
contains
  ...
  subroutine lda_init()
    ...
    do i = 1, mesh%np
      ...
      case(SPINORS)
        messages_lines(1) = 'Do not know how to handle spinors.'
        call messages_fatal(1)
      end select
    end do
