 ...
 subroutine species_atom_density_np(mesh, sb, atom, pos,  spin_channels, rho)
  ...
  subroutine species_get_density(species, namespace, pos, mesh, rho)
    ...
    case(SPECIES_FULL_DELTA)
      ...
      write(messages_lines(1), '(3a,f5.2,3a)') &
    ...
    case(SPECIES_FULL_GAUSSIAN)
      ...
      if(.not.conv) then
        write(messages_lines(1),'(a)') 'Internal error in species_get_density.'
  ...
  subroutine species_get_local(species, mesh, namespace, x_atom, vl)
    ...
    call profiling_in(prof, "SPECIES_GET_LOCAL")
      ...
      case(SPECIES_FROM_FILE)
        ...
        if(err /= 0) then
          write(messages_lines(1), '(a)')    'Error loading file '//trim(species_filename(species))//'.'
          write(messages_lines(2), '(a,i4)') 'Error code returned = ', err
