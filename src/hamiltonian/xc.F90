...
contains
  ...
  subroutine xc_write_info(xcs, iunit)
    ...
    write(message_g%lines(1), '(a)') "Exchange-correlation:"
    ...
    if(xcs%exx_coef /= M_ZERO) then
      ...
      write(message_g%lines(2), '(a,f8.5)') "Exact exchange mixing = ", xcs%exx_coef
  ...
  subroutine xc_init(xcs, namespace, ndim, periodic_dim, nel, x_id, c_id, xk_id, ck_id, hartree_fock)
    ...
    if(ll) then
      if((xcs%functional(FUNC_X,1)%id /= 0).and.(xcs%functional(FUNC_X,1)%id /= XC_OEP_X)) then
        message_g%lines(1) = "You cannot use an exchange functional when performing"
        message_g%lines(2) = "a Hartree-Fock calculation or using a hybrid functional."
        call message_g%fatal(2)
      end if

      if(periodic_dim == ndim) &
        call message_g%experimental("Fock operator (Hartree-Fock, OEP, hybrids) in fully periodic systems")
    ...
    if (bitand(xcs%family, XC_FAMILY_LCA) /= 0) &
      call message_g%not_implemented("LCA current functionals") ! not even in libxc!

    call message_g%obsolete_variable(namespace, 'MGGAimplementation')
    call message_g%obsolete_variable(namespace, 'CurrentInTau', 'XCUseGaugeIndependentKED')
  ...
  contains 
    ...
    subroutine parse()
      ...
      if(abs(xcs%kernel_lrc_alpha) > M_EPSILON) &
        call message_g%experimental("Long-range correction to kernel")
      ...
      if(xcs%xc_density_correction /= LR_NONE) then 
        call message_g%experimental('XC density correction')
      ...
      !%End
      call message_g%obsolete_variable(namespace, 'XCParallel', 'ParallelXC')
