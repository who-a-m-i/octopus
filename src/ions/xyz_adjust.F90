...
contains
  ...
  subroutine xyz_adjust_it(geo, namespace, rotate)
    ...
    if(optional_default(rotate, .true.)) then
      ...
      write(messages_lines(1),'(a,6f15.6)') 'Using main axis ', to(1:geo%space%dim)
      ...
      case(NONE, INERTIA, PSEUDO)
        ...
        write(messages_lines(1),'(3a,99f15.6)') 'Center of mass [', trim(units_abbrev(units_out%length)), '] = ', &
      ...
      case(LARGE)
        ...
        write(messages_lines(1),'(3a,99f15.6)') 'Center [', trim(units_abbrev(units_out%length)), '] = ', &
          (units_from_atomic(units_out%length, center(idir)), idir = 1, geo%space%dim)
        call messages_info(1)
      ...
      case default
        write(messages_lines(1), '(a,i2,a)') 'AxisType = ', axis_type, ' not known by Octopus.'
        call messages_fatal(1)
      end select

      write(messages_lines(1),'(a,99f15.6)') 'Found primary   axis ', x1(1:geo%space%dim)
      write(messages_lines(2),'(a,99f15.6)') 'Found secondary axis ', x2(1:geo%space%dim)
  ...
  subroutine axis_inertia(geo, x, x2, pseudo)
    ...
    if(pseudo) then
      write(messages_lines(1),'(a)') 'Moment of pseudo-inertia tensor [' // trim(units_abbrev(unit)) // ']'
    else
      write(messages_lines(1),'(a)') 'Moment of inertia tensor [amu*' // trim(units_abbrev(unit)) // ']'
    end if
    call messages_info(1)
    ...
    write(messages_lines(1),'(a,6f25.6)') 'Eigenvalues: ', &
