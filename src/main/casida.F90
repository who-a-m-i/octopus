...
contains
  ...
  subroutine casida_run(sys, hm, fromScratch)
    ...
    if(ierr == 0) then
      call states_elec_look_and_load(gs_restart, sys%parser, sys%st, sys%gr)
    ...
    call states_elec_count_pairs(sys%st, sys%parser, cas%n_pairs, cas%n_occ, cas%n_unocc, cas%is_included, is_frac_occ)
    if(is_frac_occ) then
