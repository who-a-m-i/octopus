...
contains
 ...
 ! ---------------------------------------------------------
  subroutine test_projector(param, parser)
    ...
    call hamiltonian_elec_epot_generate(sys%hm, sys%parser, sys%gr, sys%geo, sys%st)
  ...
  subroutine test_hamiltonian(param, parser)
    ...
    do itime = 1, param%repetitions
      if(states_are_real(sys%st)) then
        call dhamiltonian_elec_apply_batch(sys%hm, sys%gr%der, sys%st%group%psib(1, 1), hpsib, 1, terms = terms, set_bc = .false.)
      else
        call zhamiltonian_elec_apply_batch(sys%hm, sys%gr%der, sys%st%group%psib(1, 1), hpsib, 1, terms = terms, set_bc = .false.)
      end if
    end do
