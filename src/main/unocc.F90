...
contains
  ...
  subroutine unocc_run(sys, fromscratch)
    ...
    if (.not. fromScratch) then
      ...
      if(ierr == 0) then
        call states_elec_load(restart_load_unocc, sys%parser, sys%st, sys%gr, ierr, lowest_missing = lowest_missing)
    ...
    if(ierr_rho == 0) then
      if (read_gs) then
        call states_elec_load(restart_load_gs, sys%parser, sys%st, sys%gr, ierr, lowest_missing = lowest_missing)
        if(sys%hm%lda_u_level /= DFT_U_NONE) &
    ...
    if(simul_box_is_periodic(sys%gr%sb).and. sys%st%d%nik > sys%st%d%nspin) then
      if(bitand(sys%gr%sb%kpoints%method, KPOINTS_PATH) /= 0) then
        call states_elec_write_bandstructure(STATIC_DIR, sys%parser, sys%st%nst, sys%st, sys%gr%sb, sys%geo, sys%gr%mesh, &
  ...
  contains
    ...
    subroutine init_(mesh, st)
      ...
      call eigensolver_init(eigens, sys%parser, sys%gr, st, sys%ks%xc, sys%hm)
