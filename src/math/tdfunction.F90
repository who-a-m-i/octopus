...
100 continue
  ...
  subroutine tdf_fourier_grid(f, wgrid)
    ...
    case default
      messages_lines(1) = "Illegal mode in tdf_fourier_grid."
  ...
  subroutine tdf_set_random(f, fdotf)
    ...
    case(TDF_ZERO_FOURIER)
      n = 2*f%nfreqs-2
    case default
      messages_lines(1) = "Illegal value for f%mode in tdf_set_random."
      call messages_fatal(1)
    end select
    SAFE_ALLOCATE(e(1:n))
  ...
  FLOAT function tdf_diff(f, g) result (fg)
    ...
    case default
      messages_lines(1) = "Illegal value for f%mode in tdf_diff"
      call messages_fatal(1)
    end select
