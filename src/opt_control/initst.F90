...
contains
  ...
  subroutine initial_state_init(sys, hm, qcstate)
    ...
    case(oct_is_groundstate) 
      ...
      if(ierr == 0) call states_elec_load(restart, sys%parser, psi, sys%gr, ierr)
      if (ierr /= 0) then
    ...
    if(freeze_orbitals > 0) then
      ...
      call states_elec_freeze_orbitals(psi, sys%parser, sys%gr, sys%mc, freeze_orbitals, family_is_mgga(sys%ks%xc_family))
    ...
    elseif(freeze_orbitals < 0) then
      ...
      call density_calc(psi, sys%gr, psi%rho)
      call v_ks_calc(sys%ks, sys%parser, hm, psi, sys%geo, calc_eigenval = .true.)
      call states_elec_freeze_orbitals(psi, sys%parser, sys%gr, sys%mc, psi%nst - 1, family_is_mgga(sys%ks%xc_family))
