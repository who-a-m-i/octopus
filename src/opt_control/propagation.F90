...
contains
  ...
  subroutine propagate_forward(sys, td, par, tg, qcpsi, prop, write_iter)
    ...
    messages(1) = "Info: Forward propagation."
    ...
    if (present(prop)) then
      ...
      if (ierr /= 0) then
        messages(1) = "Unable to write OCT states restart."
    ...
    write(messages(1),'(a,f12.2,a)') 'Propagation time: ', final_time - init_time, ' seconds.'
    call message_g%info(1)
  ...
  subroutine propagate_backward(sys, td, qcpsi, prop)
    ...
    messages(1) = "Info: Backward propagation."
    call message_g%info(1)
  ...
  subroutine fwd_step(sys, td, tg, par, par_chi, qcpsi, prop_chi, prop_psi)
    ...
    if (ierr /= 0) then
      messages(1) = "Unable to read OCT states restart."
  ...
  subroutine oct_prop_check(prop, namespace, psi, gr, iter)
    ...
    do j = 1, prop%number_checkpoints + 2
     if(prop%iter(j)  ==  iter) then
       ...
       if (ierr /= 0) then
         messages(1) = "Unable to read wavefunctions from '"//trim(dirname)//"'."
         call message_g%fatal(1)
       end if
  ...
  subroutine oct_prop_dump_states(prop, iter, psi, gr, ierr)
    ...
    if (debug%info) then
      messages(1) = "Debug: Writing OCT propagation states restart."
      call message_g%info(1)
    end if
    ...
    do j = 1, prop%number_checkpoints + 2
      if(prop%iter(j)  ==  iter) then
        ...
        if (err /= 0) then
          messages(1) = "Unable to write wavefunctions to '"//trim(dirname)//"'."
          call message_g%warning(1)
          ierr = ierr + 2**j
        end if
    ...
    if (debug%info) then
      messages(1) = "Debug: Writing OCT propagation states restart done."
      call message_g%info(1)
    end if
  ...
  subroutine oct_prop_load_states(prop, namespace, psi, gr, iter, ierr)
    ...
    if (debug%info) then
      messages(1) = "Debug: Reading OCT propagation states restart."
      call message_g%info(1)
    end if
    ...
    if (debug%info) then
      messages(1) = "Debug: Reading OCT propagation states restart done."
      call message_g%info(1)
    end if
