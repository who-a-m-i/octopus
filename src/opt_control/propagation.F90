...
contains
  ...
  subroutine propagate_forward(sys, td, par, tg, qcpsi, prop, write_iter)
    ...
    if(ion_dynamics_ions_move(td%ions)) then
      call hamiltonian_elec_epot_generate(sys%hm, sys%parser,  gr, sys%geo, psi, time = M_ZERO)
  ...
  subroutine fwd_step(sys, td, tg, par, par_chi, qcpsi, prop_chi, prop_psi)
    ...
    do i = 1, td%max_iter
      ...
      call update_hamiltonian_elec_chi(i, sys%parser, gr, sys%ks, sys%hm, td, tg, par_chi, sys%geo, psi2)
      call hamiltonian_elec_update(sys%hm, gr%mesh, time = (i - 1)*td%dt)
      call propagator_dt(sys%ks, sys%parser, sys%hm, gr, chi, tr_chi, i*td%dt, td%dt, td%mu, i, td%ions, sys%geo, sys%outp)
      if(aux_fwd_propagation) then
        call update_hamiltonian_elec_psi(i, sys%parser, gr, sys%ks, sys%hm, td, tg, par_prev, psi2, sys%geo)
        call propagator_dt(sys%ks, sys%parser, sys%hm, gr, psi2, tr_psi2, i*td%dt, td%dt, td%mu, i, td%ions, sys%geo, sys%outp)
      end if
      call update_hamiltonian_elec_psi(i, sys%parser, gr, sys%ks, sys%hm, td, tg, par, psi, sys%geo)
      call hamiltonian_elec_update(sys%hm, gr%mesh, time = (i - 1)*td%dt)
  ...
  subroutine bwd_step(sys, td, tg, par, par_chi, qcchi, prop_chi, prop_psi) 
    ...
    call density_calc(psi, gr, psi%rho)
    call v_ks_calc(sys%ks, sys%parser, sys%hm, psi, sys%geo)
    call hamiltonian_elec_update(sys%hm, gr%mesh)
    call propagator_run_zero_iter(sys%hm, gr, td%tr)
    call propagator_run_zero_iter(sys%hm, gr, tr_chi)
    ...
    do i = td%max_iter, 1, -1
      ...
      call update_hamiltonian_elec_chi(i-1, sys%parser, gr, sys%ks, sys%hm, td, tg, par_chi, sys%geo, psi)
      call hamiltonian_elec_update(sys%hm, gr%mesh, time = abs(i*td%dt))
      ...
      call update_hamiltonian_elec_psi(i-1, sys%parser, gr, sys%ks, sys%hm, td, tg, par, psi, sys%geo)
      call hamiltonian_elec_update(sys%hm, gr%mesh, time = abs(i*td%dt))
  ...
  subroutine bwd_step_2(sys, td, tg, par, par_chi, qcchi, prop_chi, prop_psi) 
    ...
    do i = td%max_iter, 1, -1
      ...
      case default
        ...
        if(ion_dynamics_ions_move(td%ions)) then
          ...
          call hamiltonian_elec_epot_generate(sys%hm, sys%parser, gr, sys%geo, psi, time = abs((i-1)*td%dt))
        end if
        ...
        call update_hamiltonian_elec_chi(i-1, sys%parser, gr, sys%ks, sys%hm, td, tg, par, sys%geo, st_ref, qtildehalf)
    ...
    td%dt = -td%dt
    call update_hamiltonian_elec_psi(0, sys%parser, gr, sys%ks, sys%hm, td, tg, par, psi, sys%geo)
    call update_field(0, par_chi, gr, sys%hm, sys%geo, qcpsi, qcchi, par, dir = 'b')
  ...
  !
  ! ----------------------------------------------------------
  subroutine update_hamiltonian_elec_chi(iter, parser, gr, ks, hm, td, tg, par_chi, geo, st, qtildehalf)
  ...
  !
  ! ----------------------------------------------------------
  subroutine update_hamiltonian_elec_psi(iter, parser, gr, ks, hm, td, tg, par, st, geo)
    integer,                 intent(in)    :: iter
    type(parser_t),          intent(in)    :: parser
    type(grid_t),            intent(inout) :: gr
    type(v_ks_t),            intent(inout) :: ks
  ...
  subroutine oct_prop_check(prop, parser, psi, gr, iter)
    ...
    do j = 1, prop%number_checkpoints + 2
     if(prop%iter(j)  ==  iter) then
       ...
       if (ierr == 0) call states_elec_load(prop%restart_load, parser, stored_st, gr, ierr, verbose=.false.)
       if (ierr /= 0) then
  ...
  subroutine oct_prop_load_states(prop, parser, psi, gr, iter, ierr)
    ...
    do j = 1, prop%number_checkpoints + 2
      if (prop%iter(j)  ==  iter) then
        ...
        if (err == 0) call states_elec_load(prop%restart_load, parser, psi, gr, err, verbose=.false.)
        if (err /= 0) then
          message(1) = "Unable to read wavefunctions from '"//trim(dirname)//"'."
          call messages_warning(1)
          ierr = ierr + 2**j
