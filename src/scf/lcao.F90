...
!#define LCAO_DEBUG
  ...
  subroutine lcao_init(this, namespace, gr, geo, st)
    ...
    if(st%d%ispin == SPINORS .and. this%alternative) then
      messages(1) = "LCAOAlternative is not working for spinors."
...
! mysterious problems with optimization on PGI 12.4.0.
    ...
    subroutine lcao2_init()
      ...
      if(this%derivative) then
        ...
        if(st%nst * st%smear%el_per_state > st%qtot) then
          messages(1) = "Lower-lying empty states may be missed with LCAOExtraOrbitals."
      ...
      if(this%parallel) then      
        ...
        if(info /= 0) then
          write(messages(1), '(a,i6)') 'descinit for BLACS failed with error code ', info
          call message_g%fatal(1)
        end if
  ...
  subroutine lcao_run(sys, st_start, lmm_r)
    ...
    if(lcao_is_available(lcao)) then
      ...
      if(present(st_start)) then
        write(messages(1),'(a,i8,a)') 'Performing LCAO for states ', st_start, ' and above'
        call message_g%info(1)
      end if
    ...
    if(.not. lcao_done .or. lcao%norbs < sys%st%nst) then
      ...
      if(st_start_random > 1) then
        write(messages(1),'(a,i8,a)') 'Generating random wavefunctions for states ', st_start_random, ' and above'
        call message_g%info(1)
      end if
  ...
  subroutine lcao_guess_density(this, namespace, st, gr, sb, geo, qtot, nspin, spin_channels, rho)
    ...
    case (INITRHO_USERDEF) ! User-defined
      ...
      if(parse_block(namespace, 'AtomsMagnetDirection', blk) < 0) then
        messages(1) = "AtomsMagnetDirection block is not defined."
        call message_g%fatal(1)
      end if

      if (parse_block_n(blk) /= geo%natoms) then
        messages(1) = "AtomsMagnetDirection block has the wrong number of rows."
        call message_g%fatal(1)
      end if
    ...
    write(messages(1),'(a,f13.6)')'Info: Unnormalized total charge = ', rr
    call message_g%info(1)
    ...
    write(messages(1),'(a,f13.6)')'Info: Renormalized total charge = ', rr
    call message_g%info(1)
