...
! problems with PGI 12.4.0 -- LAPACK fails in diagonalization, only if mesh partition from scratch!
  if(this%debug .and. mpi_grp_is_root(mpi_world)) then
    iunit_h = io_open(file=STATIC_DIR//'lcao_hamiltonian', action='write', &
      namespace=this%parser%get_namespace())
    iunit_s = io_open(file=STATIC_DIR//'lcao_overlap', action='write', &
      namespace=this%parser%get_namespace())
    iunit_e = io_open(file=STATIC_DIR//'lcao_eigenvectors', action='write', &
      namespace=this%parser%get_namespace())
