...
contains
  ...
  subroutine mix_init(smix, namespace, der, d1, d2, d3, def_, func_type_, prefix_)
    ...
    call message_g%obsolete_variable(namespace, 'TypeOfMixing', 'MixingScheme')
    ...
    if(.not.varinfo_valid_option('MixingScheme', smix%scheme)) call message_g%input_error('MixingScheme', 'invalid option')
    call message_g%print_var_option(stdout, "MixingScheme", smix%scheme)

    if(smix%scheme == OPTION__MIXINGSCHEME__DIIS) call message_g%experimental('MixingScheme = diis')
    ...
    if(smix%precondition) call message_g%experimental('MixingPreconditioner')
    ...
    if(smix%coeff <= M_ZERO .or. smix%coeff > M_ONE) then
      call message_g%input_error('Mixing', 'Value should be positive and smaller than one.')
    end if
    ...
    if(smix%residual_coeff <= M_ZERO .or. smix%residual_coeff > M_ONE) then
      call message_g%input_error('MixingResidual', 'Value should be positive and smaller than one.')
    end if
    ...
    if (smix%scheme /= OPTION__MIXINGSCHEME__LINEAR) then
      ...
      if(smix%ns <= 1) call message_g%input_error('MixNumberSteps')
    else
      smix%ns = 0
    end if
    ...
    if(smix%interval < 1) call message_g%input_error('MixInterval', 'MixInterval must be larger or equal than 1')
  ...
  subroutine mix_set_mixing(smix, newmixing)
    ...
    !  message_g%lines(1) = "Mixing can only be adjusted in linear mixing scheme."
  ...
  subroutine mix_dump(restart, smix, mesh, ierr)
    ...
    if (debug%info) then
      message_g%lines(1) = "Debug: Writing mixing restart."
    ...
    if (debug%info) then
      message_g%lines(1) = "Debug: Writing mixing restart done."
      call message_g%info(1)
    end if
  ...
  subroutine mix_load(restart, smix, mesh, ierr)
    ...
    if (debug%info) then
      message_g%lines(1) = "Debug: Reading mixing restart."
      call message_g%info(1)
    end if
    ...
    if (ierr == 0) then
      ...
      if (scheme /= smix%scheme .or. ns /= smix%ns) then
        message_g%lines(1) = "The mixing scheme from the restart data is not the same as the one used in the current calculation."
      ...
      if (mesh%np_global /= d1 .or. mesh%np /= smix%mixfield%d1 .or. d2 /= smix%mixfield%d2 .or. d3 /= smix%mixfield%d3 ) then
        message_g%lines(1) = "The dimensions of the arrays from the mixing restart data"
        message_g%lines(2) = "are not the same as the ones used in this calculation."
    ...
    if (debug%info) then
      message_g%lines(1) = "Debug: Reading mixing restart done."
      call message_g%info(1)
    end if
  ...
  subroutine mix_add_auxmixfield( smix, mixfield )
    ...
    if( smix%scheme == OPTION__MIXINGSCHEME__DIIS) &
      call message_g%input_error('Mixing scheme  DIIS is not implemented for auxiliary mixing fields')

    if( smix%scheme == OPTION__MIXINGSCHEME__BOWLER_GILLAN) &
      call message_g%input_error('Mixing scheme  Bowler Gillan is not implemented for auxiliary mixing fields')
