...
contains
  ...
  subroutine rdmft_init(rdm, namespace, gr, st, ks, fromScratch)
    ...
    if(st%nst < st%qtot + 1) then   
      messages(1) = "Too few states to run RDMFT calculation"
      messages(2) = "Number of states should be at least the number of electrons plus one"
  ...
  subroutine scf_rdmft(rdm, namespace, gr, geo, st, ks, hm, psolver, outp, max_iter, restart_dump)
    ...
    if (.not. rdm%do_basis) then
      ...
      write(messages(1),'(a)') 'Calculating Coulomb and exchange matrix elements in basis'
      write(messages(2),'(a)') '--this may take a while--'
    ...
    do iter = 1, max_iter
      rdm%iter = rdm%iter + 1
      write(messages(1), '(a)') '**********************************************************************'
      write(messages(2),'(a, i4)') 'Iteration:', iter
      call message_g%info(2)
      ...
      write(messages(1), '(a)') 'Optimization of natural orbitals'
      ...
      write(messages(1),'(a,11x,es20.10)') 'Total energy:', units_from_atomic(units_out%energy,energy + hm%ep%eii) 
      write(messages(2),'(a,1x,es20.10)') 'Rel. energy difference:', rel_ener
      call message_g%info(2)

      if (.not. rdm%hf .and. rdm%do_basis) then
        write(messages(1),'(a,18x,es20.10)') 'Max F0:', rdm%maxFO
        call message_g%info(1)
      end if
    ...
    if(conv) then 
      write(messages(1),'(a,i3,a)')  'The calculation converged after ',rdm%iter,' iterations'
      write(messages(2),'(a,9x,es20.10)')  'The total energy is ', units_from_atomic(units_out%energy,energy + hm%ep%eii)
      ...
      write(messages(1),'(a,i3,a)')  'The calculation did not converge after ', iter-1, ' iterations '
      write(messages(2),'(a,es15.5)') 'Relative energy difference between the last two iterations ', rel_ener
      write(messages(3),'(a,es15.5)') 'The maximal non-diagonal element of the Hermitian matrix F is ', rdm%maxFO
      call message_g%info(3)
    end if
  ...
  subroutine scf_occ_NO(rdm, gr, hm, psolver, st, energy)
    ...
    write(messages(1),'(a)') 'SKIP Optimization of occupation numbers'
    call message_g%info(1)
    ...
    write(messages(1),'(a4,5x,a12)')'#st','Occupation'
    call message_g%info(1)   

    do ist = 1, st%nst
      write(messages(1),'(i4,3x,f11.9)') ist, st%occ(ist, 1)
      call message_g%info(1)
    end do

    write(messages(1),'(a,1x,f13.9)') 'Sum of occupation numbers', rdm%occsum
    write(messages(2),'(a,es20.10)') 'Total energy occ', units_from_atomic(units_out%energy,energy + hm%ep%eii) 
    call message_g%info(2)   
  ...
  subroutine scf_occ(rdm, gr, hm, psolver, st, energy)
    ...
    write(messages(1),'(a)') 'Optimization of occupation numbers'
    call message_g%info(1)
    ...
    if (icycle >= 50) then
      write(messages(1),'(a,1x,f11.4)') 'Bisection ended without finding mu, sum of occupation numbers:', rdm%occsum
    ...
    do ist = 1, st%nst
      write(messages(1),'(i4,3x,f14.12)') ist, st%occ(ist, 1)
      call message_g%info(1)
    end do

    write(messages(1),'(a,3x,f11.9)') 'Sum of occupation numbers: ', rdm%occsum
    write(messages(2),'(a,11x,es20.10)') 'Total energy: ', units_from_atomic(units_out%energy, energy + hm%ep%eii)
    call message_g%info(2)   
