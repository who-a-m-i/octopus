...
contains
  ...
  subroutine scf_init(scf, namespace, gr, geo, st, mc, hm, ks, conv_force)
    ...
    if(hm%lda_u_level /= DFT_U_NONE .and. hm%lda_u%basisfromstates) then
      ...
      if (ierr /= 0) then
        messages(1) = "Unable to load LDA+U basis from selected states."
    ...
    if(scf%lcao_restricted) then
      ...
      messages(1) = 'Info: SCF restricted to LCAO subspace.'
      call message_g%info(1)

      if(scf%conv_eigen_error) then
        messages(1) = "ConvEigenError cannot be used with SCFinLCAO, since error is unknown."
        call message_g%fatal(1)
      end if
    end if
  ...
  subroutine scf_run(scf, namespace, mc, gr, geo, st, ks, hm, psolver, outp, gs_run, verbosity, iters_done, &
    ...
    if(scf%forced_finish) then
      messages(1) = "Previous clean stop, not doing SCF and quitting."
    ...
    if(scf%lcao_restricted) then
      ...
      if(.not. lcao_is_available(lcao)) then
        messages(1) = 'LCAO is not available. Cannot do SCF in LCAO.'
        call message_g%fatal(1)
      end if
    end if
    ...
    if (present(restart_load)) then
      if (restart_has_flag(restart_load, RESTART_FLAG_RHO)) then
        ...
        if (ierr /= 0) then
          messages(1) = 'Unable to read density. Density will be calculated from states.'
      ...
      if (restart_has_flag(restart_load, RESTART_FLAG_VHXC)) then
        ...
        if (ierr /= 0) then
          messages(1) = 'Unable to read Vhxc. Vhxc will be calculated from states.'
          call message_g%warning(1)
        else
      ...
      if (restart_has_flag(restart_load, RESTART_FLAG_MIX)) then
        ...
        if (ierr /= 0) then
          messages(1) = "Unable to read mixing information. Mixing will start from scratch."
          call message_g%warning(1)
        end if
      end if
      ...
      if(hm%lda_u_level /= DFT_U_NONE) then
        ...
        if (ierr /= 0) then
          messages(1) = "Unable to read LDA+U information. LDA+U data will be calculated from states."
          call message_g%warning(1)
        end if
      end if 
    end if
    ...
    if ( verbosity_ /= VERB_NO ) then
      if(scf%max_iter > 0) then
        write(messages(1),'(a)') 'Info: Starting SCF iteration.'
      else
        write(messages(1),'(a)') 'Info: No SCF iterations will be done.'
    ...
    do iter = 1, scf%max_iter
      ...
      if(scf%lcao_restricted) then
        ...
        if(associated(hm%vberry)) then
          ...
          do iberry = 1, scf%max_iter_berry
            ...
            write(messages(1),'(a,9f12.6)') 'Dipole = ', dipole(1:gr%sb%dim)
            call message_g%info(1)
      ...
      case (OPTION__MIXFIELD__DENSITY)
        ...
        if(minval(st%rho(1:gr%fine%mesh%np, 1:st%d%spin_channels)) < -CNST(1e-6)) then
          write(messages(1),*) 'Negative density after mixing. Minimum value = ', &
      ...
      if (gs_run_ .and. present(restart_dump)) then 
        ...
        if ( (finish .or. (modulo(iter, outp%restart_write_interval) == 0) &
          ...
          if (ierr /= 0) then
            messages(1) = 'Unable to write density.'
            call message_g%warning(1)
          end if
          ...
          if(hm%lda_u_level /= DFT_U_NONE) then
            ...
            if (ierr /= 0) then
              messages(1) = 'Unable to write LDA+U information.'
              call message_g%warning(1)
            end if
          end if
          ...
          case (OPTION__MIXFIELD__DENSITY)
            ...
            if (ierr /= 0) then
              messages(1) = 'Unable to write mixing information.'
              call message_g%warning(1)
            end if
          case (OPTION__MIXFIELD__POTENTIAL)
            call hamiltonian_elec_dump_vhxc(restart_dump, hm, gr%mesh, ierr)
            if (ierr /= 0) then
              messages(1) = 'Unable to write Vhxc.'
              call message_g%warning(1)
            end if
      ...
      if(finish) then
        ...
        if(verbosity_ >= VERB_COMPACT) then
          write(messages(1), '(a, i4, a)') 'Info: SCF converged in ', iter, ' iterations'
          write(messages(2), '(a)')        '' 
    ...
    if(scf%max_iter > 0 .and. any(scf%eigens%converged < st%nst) .and. .not. scf%lcao_restricted) then
      write(messages(1),'(a)') 'Some of the states are not fully converged!'
      call message_g%warning(1)
    end if

    if(.not.finish) then
      write(messages(1), '(a,i4,a)') 'SCF *not* converged after ', iter - 1, ' iterations.'
      call message_g%warning(1)
    end if
  ...
  contains
    ...
    subroutine scf_write_iter()
      ...
      if ( verbosity_ == VERB_FULL ) then
        ...
        write(messages(1),'(a,es15.8,2(a,es9.2))') ' etot  = ', units_from_atomic(units_out%energy, hm%energy%total), &
          ' abs_ev   = ', units_from_atomic(units_out%energy, scf%abs_ev), ' rel_ev   = ', scf%rel_ev
        write(messages(2),'(a,es15.2,2(a,es9.2))') &
        ...
        if(.not.scf%lcao_restricted) then
          write(messages(1),'(a,i6)') 'Matrix vector products: ', scf%eigens%matvec
          write(messages(2),'(a,i6)') 'Converged eigenvectors: ', sum(scf%eigens%converged(1:st%d%nik))
          call message_g%info(2)
        ...
        write(messages(1),'(a)') ''
        write(messages(2),'(a,i5,a,f14.2)') 'Elapsed time for SCF step ', iter,':', etime
        call message_g%info(2)
        ...
        if(conf%report_memory) then
          ...
          mem = mem_tmp
#endif
          write(messages(1),'(a,f14.2)') 'Memory usage [Mbytes]     :', mem
          call message_g%info(1)
        end if
      ...
      if ( verbosity_ == VERB_COMPACT ) then
        ! write info about forces only if they are used as convergence criteria
        if (scf%conv_abs_force > M_ZERO) then
          write(messages(1),'(a,i4,a,es15.8, 2(a,es9.2), a, f7.1, a)') &
            'iter ', iter, &
          ...
          write(messages(1),'(a,i4,a,es15.8, a,es9.2, a, f7.1, a)') &
            'iter ', iter, &
            ' : etot ', units_from_atomic(units_out%energy, hm%energy%total), &
            ' : abs_dens', scf%abs_dens, &
            ' : etime ', etime, 's'
