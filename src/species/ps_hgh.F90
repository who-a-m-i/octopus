...
contains
  ...
  subroutine hgh_process(psp)
    ...
    if(ierr /= 0) then ! If the wavefunctions could not be found, we set its number to zero.
      ...
      write(messages(2),'(a)') 'do its job. The program will continue, but expect poor'
      write(messages(3),'(a)') 'convergence properties.'
    ...
    if(ierr  ==  0) then
      ...
      do n = 1, psp%conf%p
        ...
        if (e > CNST(1.0e-5)) then
          write(messages(1), '(a,i2,a)') "Eigenstate for n = ", n , ' is not normalized'
          write(messages(2), '(a, f12.6,a)') '(abs(1-norm) = ', e, ')'
