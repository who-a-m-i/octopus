...
!!
  ...
  subroutine ps_pspio_init(ps, namespace, label, z, lmax, lloc, ispin, filename)
    ...
    if(.not. found) then
      do idir = 1, size(psp_dir)
        ...
        if (idir == size(psp_dir)) then
          messages(1) = "Pseudopotential file '" // trim(filename) // " not found"
          call message_g%fatal(1)
        end if
      end do
    end if

    messages(1) = "Reading pseudopotential from file:"
    write(messages(2), '(6x,3a)') "'", trim(filename2), "'"
    ...
    lmax = ps%lmax
    write(messages(1), '(a,i2,a)') "Info: l = ", ps%lmax, " is maximum angular momentum considered."
...
#else
    messages(1) = 'PSPIO selected for pseudopotential parsing, but the code was compiled witout PSPIO support.'
    call message_g%fatal(1)
#endif
  ...
  subroutine ps_pspio_read_mesh(ps, pspdata)
    ...
    if(any(abs(r_tmp(2:ps%g%nrval)) < M_EPSILON)) then
      ...
      messages(1) = "Illegal zero values in PSPIO radial grid"
      call message_g%fatal(1)
    end if
  ...
  subroutine ps_pspio_read_potentials(ps, pspdata)
    ...
    messages(1) = "Not yet implemented"
    call message_g%fatal(1)
