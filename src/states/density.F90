...
contains
  ...
  subroutine states_elec_freeze_orbitals(st, parser, gr, mc, n, family_is_mgga)
    ...
    call states_elec_freeze_redistribute_states(st, parser, gr, mc, n)
  ...
  ! ---------------------------------------------------------
  subroutine states_elec_freeze_redistribute_states(st, parser, gr, mc, nn)
    type(states_elec_t),  intent(inout) :: st
    type(parser_t),       intent(in)    :: parser
    type(grid_t),         intent(in)    :: gr
    type(multicomm_t),    intent(in)    :: mc
    ...
    call states_elec_distribute_nodes(st, parser, mc)
