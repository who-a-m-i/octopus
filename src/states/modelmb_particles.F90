...
contains
  ...
  subroutine modelmb_particles_init(this, namespace, gr)
    ...
    call message_g%print_var_value(stdout, "NParticleModelmb", this%nparticle)
    ...
    call message_g%print_var_value(stdout, "NDimModelmb", this%ndim)
    ...
    if(parse_block(namespace, 'DescribeParticlesModelmb', blk) == 0) then
      ...
      call message_g%experimental("Model many-body")
      
      ncols = parse_block_cols(blk, 0)
      if(ncols /= 5 ) then
        call message_g%input_error("DescribeParticlesModelmb")
      end if
