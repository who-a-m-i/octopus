...
contains
  ...
  ! ---------------------------------------------------------
  subroutine states_elec_write_tpa(dir, parser, gr, st)
  ...
  subroutine states_elec_write_bandstructure(dir, parser, nst, st, sb, geo, mesh, phase, vec_pot, vec_pot_var)
    character(len=*),  intent(in)             :: dir
    type(parser_t),               intent(in)  :: parser
    ...
    do ik = st%d%nik-npath+1, st%d%nik, ns
      do is = 0, ns - 1
        ...
        write(iunit(is),'(f14.8)',advance='no') kpoints_get_path_coord(sb%kpoints, & 
                                                   states_elec_dim_get_kpoint_index(st%d, ik + is)-(st%d%nik -npath)) 
        do idir = 1, sb%dim
