...
contains
    ...
    if(parse_block(namespace, 'MaxwellFieldsCoordinate', blk) == 0) then
      ...
      do il=1, nlines
        ...
        if (ncols < 3 .or. ncols > 3) then
            message_g%lines(1) = 'MaxwellFieldCoordinate must have 3 columns.'
  ...
  subroutine states_mxll_allocate(st, mesh)
    ...
    call batch_init(st%rs_curr_dens_rest2b, st%d%dim, 1, message_g)
...
!    Another alternative
!    call batch_init(st%rs_state_transb, hm%d%dim, 1, message_g)
