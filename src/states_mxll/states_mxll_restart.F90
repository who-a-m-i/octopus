...
contains
  ...
  subroutine states_mxll_read_user_def(mesh, st, user_def_rs_state, namespace)
    ...
    if(parse_block(namespace, 'UserDefinedInitialMaxwellStates', blk) == 0) then
      ...
      !call message_g%print_stress(stdout, trim('Substitution of the electromagnetic fields'))

      ! find out how many lines (i.e. states) the block has
      nlines = parse_block_n(blk)

      write(messages(1), '(a,i5)') 'Maxwell electromagnetic fields are added.'
      write(messages(2), '(a,i5)') ''
      ...
      do il = 1, nlines
        ...
        if(ncols  <  4 .or. ncols > 4) then
          messages(1) = 'Each line in the UserDefinedMaxwellStates block must have'
          messages(2) = 'four columns.'
        ...
        case(STATE_FROM_FORMULA)
          ...
          if (maxwell_field == OPTION__USERDEFINEDINITIALMAXWELLSTATES__ELECTRIC_FIELD) then
            ...
            call message_g%write("  E-field in dimension "//trim(cdim)//" : "//trim(st%user_def_e_field(idim)), fmt='(a,i1,2a)')
            ...
            call message_g%write("  B-field in dimension "//trim(cdim)//" : "//trim(st%user_def_b_field(idim)), fmt='(a,i1,2a)')
        ...
        case(STATE_FROM_FILE)
          ...
          if (maxwell_field == OPTION__USERDEFINEDINITIALMAXWELLSTATES__ELECTRIC_FIELD) then
            ...
            call message_g%write("  E-field in dimension "//trim(cdim)//" : "//trim(filename_e_field), fmt='(a,i1,2a)')
            ...
            call message_g%write("  B-field in dimension "//trim(cdim)//" : "//trim(filename_b_field), fmt='(a,i1,2a)')
        ...
        case default
          messages(1) = 'Wrong entry in UserDefinedMaxwellStates, column 2.'
      ...
      messages(1) = "'UserDefineInitialdStates' has to be specified as block."
  ...
  subroutine states_mxll_dump(restart, st, gr, zff, zff_dim, ierr, iter, st_start_writing, verbose)
    ...
    if(verbose_) then
      messages(1) = "Info: Writing Maxwell states."
    ...
    if(verbose_) then
      messages(1) = "Info: Finished writing Maxwell states."
      call print_date(trim(messages(1))//' ')
    end if
  ...
  subroutine states_mxll_load(restart, st, gr, zff, zff_dim, ierr, iter, lowest_missing, label, verbose)
    ...
    messages(1) = 'Info: Reading Maxwell states'
    ...
    messages(1) = 'Info: Maxwell states reading done.'
    if(verbose_) call print_date(trim(messages(1))//' ')
