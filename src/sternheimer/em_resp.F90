...
contains
  ...
  subroutine em_resp_run(sys, fromScratch)
    ...
    if(ierr == 0) then
      call states_elec_look_and_load(gs_restart, sys%parser, sys%st, sys%gr, is_complex = complex_response)
    ...
    if (use_kdotp) then
      ...
      do idir = 1, gr%sb%periodic_dim
        ...
        if (ierr == 0) call states_elec_load(kdotp_restart, sys%parser, sys%st, sys%gr, ierr, lr=kdotp_lr(idir, 1))
