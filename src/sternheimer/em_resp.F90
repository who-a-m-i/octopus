...
contains
  ...
  subroutine em_resp_run(sys, fromScratch)
    ...
    if(em_vars%lrc_kernel .and. gr%sb%periodic_dim < gr%sb%dim) then
      messages(1) = 'The use of the LRC kernel for non-periodic dimensions makes no sense.'
    ...
    if (states_are_real(sys%st)) then
      messages(1) = 'Info: Using real wavefunctions.'
    else
      messages(1) = 'Info: Using complex wavefunctions.'
    end if
    ...
    if(use_kdotp .and. .not. smear_is_semiconducting(sys%st%smear)) then
      ...
      messages(1) = "em_resp with kdotp can only be used with semiconducting smearing"
      call message_g%fatal(1)
    end if

    ! read kdotp wavefunctions if necessary
    if (use_kdotp) then
      messages(1) = "Reading kdotp wavefunctions for periodic directions."
      call message_g%info(1)

      call restart_init(kdotp_restart, sys%namespace, RESTART_KDOTP, RESTART_TYPE_LOAD, sys%mc, ierr, mesh=sys%gr%mesh)
      if(ierr /= 0) then
        messages(1) = "Unable to read kdotp wavefunctions."
        messages(2) = "Previous kdotp calculation required."
      ...
      do idir = 1, gr%sb%periodic_dim
        ...
        if(ierr /= 0) then
          messages(1) = "Could not load kdotp wavefunctions from '"//trim(wfs_tag_sigma(str_tmp, 1))//"'"
          messages(2) = "Previous kdotp calculation required."
          call message_g%fatal(2)
        end if
      end do
    ...
    if(em_vars%calc_magnetooptics) then 
      if(em_vars%calc_hyperpol .and. use_kdotp) then
        messages(1) = "Hyperpolarizability and magnetooptics with kdotp are not compatible."
        messages(2) = "Only calculation of hyperpolarizability will be performed."
    ...
    if(em_vars%calc_magnetooptics .or. &
      ...
      if(use_kdotp) then
        ...
        if(gr%sb%periodic_dim < gr%sb%dim) then
          if(pert_type(em_vars%perturbation) == PERTURBATION_MAGNETIC) then
            messages(1) = "All directions should be periodic for magnetic perturbations with kdotp."
          else
            messages(1) = "All directions should be periodic for magnetooptics with kdotp."
          end if
          call message_g%fatal(1)
        end if
    ...
    if(em_vars%lrc_kernel .and. (.not. sternheimer_add_hartree(sh)) &
      ...
      messages(1) = "Only the G = G'= 0 term of the LRC kernel is taken into account."
      call message_g%warning(1)
    end if   
    ...
    do iomega = 1, em_vars%nomega
      ...
      do ifactor = 1, em_vars%nfactor
        ...
        if(abs(frequency) < M_EPSILON .and. em_vars%calc_magnetooptics .and. use_kdotp) then
          messages(1) = "Magnetooptical response with kdotp requires non-zero frequency."
          call message_g%warning(1)       
        end if
  ...
  contains
    ...
    subroutine parse_input()
      ...
      if(parse_block(sys%namespace, 'EMFreqs', blk) == 0) then 
        ...
        do irow = 0, nrow-1
          ...
          if(nfreqs_in_row < 1) then
            messages(1) = "EMFreqs: invalid number of frequencies."
            call message_g%fatal(1)
          end if
      ...
      if(em_vars%eta < -M_EPSILON) then
        messages(1) = "EMEta cannot be negative."
        call message_g%fatal(1)
      end if
      ...
      if(pert_type(em_vars%perturbation) == PERTURBATION_ELECTRIC) then
        ...
        if (parse_block(sys%namespace, 'EMHyperpol', blk) == 0) then 
          ...
          if(abs(sum(em_vars%freq_factor(1:3))) > M_EPSILON) then
            messages(1) = "Frequency factors specified by EMHyperpol must sum to zero."
            call message_g%fatal(1)
          end if
      ...
      if(em_vars%occ_response .and. .not. (smear_is_semiconducting(sys%st%smear) .or. sys%st%smear%method == SMEAR_FIXED_OCC)) then
        messages(1) = "EMOccupiedResponse cannot be used if there are partial occupations."
        call message_g%fatal(1)
      end if
    ...
    subroutine info()
      ...
      if(pert_type(em_vars%perturbation) == PERTURBATION_ELECTRIC) then
        if(em_vars%calc_hyperpol) then 
          write(messages(1),'(a)') 'Linear-Response First-Order Hyperpolarizabilities'
          call message_g%print_stress(stdout, trim(messages(1)))
        else 
          write(messages(1),'(a)') 'Linear-Response Polarizabilities'
          call message_g%print_stress(stdout, trim(messages(1)))
        end if
      else
        write(messages(1),'(a)') 'Magnetic Susceptibilities'
        call message_g%print_stress(stdout, trim(messages(1)))
      end if

      if (states_are_real(sys%st)) then 
        messages(1) = 'Wavefunctions type: Real'
      else
        messages(1) = 'Wavefunctions type: Complex'
      end if
      call message_g%info(1)

      write(messages(1),'(a,i3,a)') 'Calculating response for ', em_vars%nomega, ' frequencies.'
      call message_g%info(1)
  ...
  !! Phys. Chem. Chem. Phys. 11, 4481 (2009)
    subroutine out_circular_dichroism()
      ...
      if(states_are_complex(st) .and. em_vars%nsigma == 2) then       
        ...
        messages(1) = "Info: Calculating rotatory response."
        call message_g%info(1)
