        ...
        do sigma = 1, em_vars%nsigma
          if(sigma == 2 .and. abs(frequency) < M_EPSILON) then
            ...
            if((.not. em_vars%calc_magnetooptics) .or. complex_wfs .or. ifactor == 1) then
              ...
              if (ierr == 0) then
                call states_elec_load(restart_load, sys%parser, sys%st, sys%gr, ierr, lr=em_vars%lr(idir, sigma_alt, ifactor))
              end if
                  ...
                  do idir2 = 1, sys%gr%sb%dim 
                    if((ipert == PKB) .or. (idir2 <= idir)) then
                      ...
                      if (ierr == 0) then
                        select case(ipert)
                          case(PK2)
                            call states_elec_load(restart_load, sys%parser, sys%st, sys%gr, ierr, &
    ...
    do idir = 1, sys%gr%sb%dim
      ...
      if(em_vars%calc_hyperpol .and. use_kdotp) then
        ...
        do idir2 = 1, gr%sb%periodic_dim
          ...
          if (ierr == 0) call states_elec_load(kdotp_restart, sys%parser, sys%st, sys%gr, ierr, lr=kdotp_lr2)
