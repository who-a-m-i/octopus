...
contains
  ...
  subroutine kdotp_lr_run(sys, fromScratch)
    ...
    if(ierr == 0) then
      call states_elec_look_and_load(restart_load, sys%parser, sys%st, sys%gr, is_complex = complex_response)
    ...
    do idir = 1, pdim
      ...
      if(.not. fromScratch) then
        ...
        if (ierr == 0) call states_elec_load(restart_load, sys%parser, sys%st, sys%gr, ierr, lr=kdotp_vars%lr(1, idir))
        ...
        if(calc_2nd_order) then
          do idir2 = idir, pdim
            ...
            call restart_open_dir(restart_load, wfs_tag_sigma(str_tmp, 1), ierr)
            if (ierr == 0) call states_elec_load(restart_load, sys%parser, sys%st, sys%gr, ierr, lr=kdotp_vars%lr2(1, idir, idir2))
            call restart_close_dir(restart_load)
