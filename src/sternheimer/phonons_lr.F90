...
contains
  ...
  subroutine phonons_lr_run(sys, hm, fromscratch)
    ...
    if(ierr == 0) then
      call states_elec_look_and_load(gs_restart, sys%parser, st, gr)
    ...
    if (simul_box_is_periodic(gr%sb) .and. do_infrared) then
      ...
      do idir = 1, gr%sb%periodic_dim
        ...
        if (ierr == 0) call states_elec_load(kdotp_restart, sys%parser, sys%st, sys%gr, ierr, lr=kdotp_lr(idir))
    ...
    do imat = start_mode, vib%num_modes
      ...
      if (.not. fromscratch) then
        ...
        if (ierr == 0) call states_elec_load(restart_load, sys%parser, st, gr, ierr, lr = lr(1))
        if (ierr /= 0) then
