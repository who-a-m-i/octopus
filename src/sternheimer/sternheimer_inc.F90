...
subroutine X(sternheimer_solve)(                           &
  ...
  iter_loop: do iter = 1, this%scf_tol%max_iter
    if (calculate_rho) then
      write(messages(1), '(a, i3)') "LR SCF Iteration: ", iter
      call message_g%info(1)
    end if

    write(messages(1), '(a, f20.6, a, f20.6, a, i1)') &
      ...
      do ib = st%group%block_start, st%group%block_end
        ...
        do ist = sst, est
          do sigma = 1, nsigma
            ...
            write(messages(1), '(i5, i5, f20.6, i8, e20.6)') &
    ...
    if(calculate_rho) then
      ...
      if (ierr /= 0) then
        messages(1) = "Unable to write response density '"//trim(rho_tag)//"'."
    ...
    do sigma = 1, nsigma 
      ...
      if (err /= 0) then
        messages(1) = "Unable to write response wavefunctions."
        call message_g%warning(1)
      end if
    ...
    if (.not. states_conv) then
      messages(1) = "Linear solver failed to converge all states."
      call message_g%warning(1)
    end if
    ...
    if(this%scf_tol%max_iter == iter) then 
      messages(1) = "Self-consistent iteration for response did not converge."
      this%ok = .false.
      call message_g%warning(1)
    end if
    ...
    write(messages(1), '(a,e16.6,a,e16.6,a)') "SCF Residual: ", abs_dens, " (abs), ", rel_dens, " (rel)"
...
!        write(messages(1),*) 'dl_eig1 ist ', ist, 'ik ', ik, dl_eig1
!        write(messages(2),*) 'dl_eig2 ist ', ist, 'ik ', ik, dl_eig2
!        call message_g%info(2)
