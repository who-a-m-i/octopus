...
contains
  ...
  contains
    ...
    subroutine init_()
      ...
      if(ierr == 0) then
        call states_elec_look_and_load(gs_restart, sys%parser, sys%st, sys%gr, is_complex = .true.)
      ...
      if (.not. fromScratch) then
        ...
        do dir = 1, ndir
          ...
          if (ierr == 0) call states_elec_load(restart_load, sys%parser, sys%st, sys%gr, ierr, lr=lr(dir,1))          
          if(ierr /= 0) then
