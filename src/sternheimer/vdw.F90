...
contains
  ...
  contains
    ...
    subroutine init_()
      ...
      if(.not.fromScratch .and. file_exists) then
        ...
        if(ii /= gaus_leg_n) then
          messages_lines(1) = "Invalid restart of van der Waals calculation."
          messages_lines(2) = "The number of points in the Gauss-Legendre integration changed."
          write(messages_lines(3), '(i3,a,i3,a)') gaus_leg_n, " (input) != ", ii, "(restart)"
      ...
      if(ierr == 0) then
        ...
        messages_lines(1) = "Previous gs calculation required."
      ...
      if (.not. fromScratch) then
        ...
        do dir = 1, ndir
          ...
          if(ierr /= 0) then
            messages_lines(1) = "Unable to read response wavefunctions from '"//trim(dirname)//"'."
    ...
    FLOAT function get_pol(omega)
      ...
      do dir = 1, ndir
        write(messages_lines(1), '(3a,f7.3)') 'Info: Calculating response for the ', index2axis(dir), &
