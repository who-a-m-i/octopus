...
contains
  ...
  subroutine current_init(this, namespace)
    ...
    if(.not.varinfo_valid_option('CurrentDensity', this%method)) call message_g%input_error('CurrentDensity')
    if(this%method /= CURRENT_GRADIENT_CORR) then
      call message_g%experimental("CurrentDensity /= gradient_corrected")
    end if
