...
subroutine X(lobpcg)(gr, st, hm, psolver, st_start, st_end, psi, constr_start, constr_end,  &
  ...
  if(no_bof) then
    messages(1) = 'Problem: orthonormalization of initial vectors failed.'
    call message_g%warning(1)
  end if

  ! Get initial Ritz-values and -vectors.
  call batch_init(psib, st%d%dim, st_start, st_end, psi(:, :, st_start:), message_g)
  call batch_init(hpsib, st%d%dim, st_start, st_end, h_psi(:, :, st_start:), message_g)
  ...
  if(no_bof) then
    messages(1) = 'Problem: Rayleigh-Ritz procedure for initial vectors failed.'
    call message_g%warning(1)
  end if
  ...
  iteration: do iter = 1, maxiter-1 ! One iteration was used up to get initial Ritz-vectors.
    ...
    if(no_bof) then
      messages(1) = 'Big problem: orthonormalization of residuals failed.'
      messages(2) = 'Quitting eigensolver iteration.'
      write(messages(3), '(a,i6)') 'in iteration #', iter
    ...
    if(lnuc > 0) then
      call batch_init(psib, st%d%dim, lnuc, message_g)
      call batch_init(hpsib, st%d%dim, lnuc, message_g)
    end if
    ...
    if(iter > 1) then
      ...
      if(no_bof) then
        messages(1) = 'Problem: orthonormalization of conjugate directions failed'
        write(messages(2), '(a,i6)') 'in iteration #', iter
    ...
    if(no_bof) then
      messages(1) = 'Problem: Rayleigh-Ritz procedure failed'
      write(messages(2), '(a,i6)') 'in iteration #', iter
      call message_g%warning(2)
      exit iteration
    end if
...
contains
  ...
  subroutine X(lobpcg_unconv_ev)()
    ...
    do i = 1, lnuc
      ...
      if(debug%info) then
        write(messages(1), '(a,i4,a,i4,a,i4,a,es12.6)') &
