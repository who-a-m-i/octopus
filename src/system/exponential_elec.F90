...
contains
  ...
  !! \f]
  ! ---------------------------------------------------------
  subroutine exponential_elec_apply(te, der, psolver, zpsi, ist, ik, deltat, order, vmagnus, imag_time)
    ...
    if (te%exp_method == EXP_TAYLOR .or. &
     ...
     !will be treated as a phase correction in the Hamiltonian
      ...
      do ii = 1, psib%nst
        ...
        call exponential_elec_apply(te, der, psolver, psi, ist, ik, deltat)
        if (batch_status(psib) /= BATCH_NOT_PACKED) then
        ...
        if(present(psib2)) then
          ...
          call exponential_elec_apply(te, der, psolver, psi2, ist, ik, deltat2)
          if (batch_status(psib2) /= BATCH_NOT_PACKED) then
