...
subroutine output_etsf(st, gr, geo, dir, outp)
  ...
  !Create a cube
  call cube_init(dcube, gr%mesh%idx%ll, gr%sb, outp%namespace, message_g, &
    fft_type=FFT_REAL, dont_optimize = .true.)
  call cube_init(zcube, gr%mesh%idx%ll, gr%sb, outp%namespace, message_g, &
...
subroutine output_etsf_error(error_data)
  ...
  messages(1) = "ETSF_IO returned a fatal error. See message above."
...
subroutine output_etsf_basisdata_write(mesh, shell, ncid)
  ...
  if((maxval(mesh%spacing(1:3)) - minval(mesh%spacing(1:3))) > CNST(1e-10)) then
    messages(1) = 'Cannot generate a ETSF plane-wave wave-functions file,'
    messages(2) = 'spacing is not the same for each direction.'
