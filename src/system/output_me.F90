...
contains
  ...
  subroutine output_me(this, dir, st, gr, geo, hm, psolver, namespace)
    ...
    if(bitand(this%what, output_me_one_body) /= 0) then
      messages(1) = "Computing one-body matrix elements"
    ...
    if(bitand(this%what, output_me_two_body) /= 0) then
      messages(1) = "Computing two-body matrix elements"
      call message_g%info(1)
  ...
  subroutine output_me_out_momentum(fname, st, gr, namespace)
    ...
    write(messages(1),'(a)') 'Momentum of the KS states [a.u.]:'
    call message_g%info(1, iunit)      
    if (st%d%nik > ns) then
      messages(1) = 'k-points [' // trim(units_abbrev(unit_one/units_out%length)) // ']'
      call message_g%info(1, iunit)
    end if
    ...
    do ik = 1, st%d%nik, ns
      ...
      if(st%d%nik > ns) then
        write(messages(1), '(a,i4, a)') '#k =', ik, ', k = ('
        do idir = 1, gr%sb%dim
      ...
      write(messages(1), '(a4,1x,a5)') '#st',' Spin'
      do idir = 1, gr%sb%dim
      ...
      do ist = 1, st%nst
        do is = 0, ns-1
          ...
          write(messages(1), '(i4,3x,a2,1x)') ist, trim(cspin)
          do idir = 1, gr%sb%dim
  ...
  subroutine output_me_out_ang_momentum(fname, st, gr, namespace)
   ...
   if(st%parallel_in_states) then
...
#endif
      write(messages(1), '(a4,1x,a5,4a12,4x,a12,1x)')       &
    ...
    write(messages(1),'(a)') 'Total Angular Momentum L [dimensionless]'
    write(messages(2),'(10x,4f12.6)') angular(1:3), lsquare
