...
subroutine X(subspace_diag_scalapack)(der, st, hm, psolver, ik, eigenval, psi, diff)
  ...
  if(info /= 0) then
    write(messages(1), '(a,i6)') "subspace diagonalization descinit for psi failed with error code ", info
  ...
  if(info /= 0) then
    write(messages(1), '(a,i6)') "subspace diagonalization descinit for Hamiltonian failed with error code ", info
    call message_g%fatal(1)
  end if
  ...
  if (elpa_init(20170403) /= elpa_ok) then
    write(messages(1),'(a)') "ELPA API version not supported"
    call message_g%fatal(1)
  endif
  elpa => elpa_allocate()
  ...
  if (info /= elpa_ok) then
    write(messages(1),'(a,i6,a,a)') "Error in ELPA, code: ", info, ", message: ", &
      elpa_strerr(info)
    call message_g%fatal(1)
  end if
...
! Use ScaLAPACK function if ELPA not available
  ...
  if(info /= 0) then
    write(messages(1),'(a,i6)') "ScaLAPACK pzheev workspace query failure, error code = ", info
    call message_g%fatal(1)
  end if
  ...
  if(info /= 0) then
    write(messages(1),'(a,i6)') "ScaLAPACK pzheev call failure, error code = ", info
    call message_g%fatal(1)
  end if
  ...
  if(info /= 0) then
    write(messages(1),'(a,i6)') "ScaLAPACK pdsyev call failure, error code = ", info
    call message_g%fatal(1)
  end if
