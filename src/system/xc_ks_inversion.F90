...
!      call messages_not_implemented("Kohn-Sham inversion in parallel")
  ...
  subroutine invertks_2part(target_rho, nspin, aux_hm, psolver, gr, st, eigensolver, namespace, asymptotics)
    ...
    if(any(target_rho(:,:) < -M_EPSILON)) then
      write(messages_lines(1),*) "Target density has negative points. min value = ", minval(target_rho(:,:))
...
!TODO: parallelize these loops over np
      ...
      ! based on residual in density
        ...
        write(messages_lines(1),'(a,2E15.4,3I8, 2E15.4)') &
...
!TODO: parallelize these loops over np
    ...
    write(messages_lines(1),'(a,I8)') "Iterative KS inversion, iterations needed:", counter
    call messages_info(1)
  ...
  subroutine xc_ks_inversion_calc(ks_inversion, namespace, gr, hm, psolver, st, vxc, time)
    ...
    if(present(time)) then
      write(messages_lines(1),'(A,F18.12)') 'xc_ks_inversion_calc - time:', time
      call messages_info(1)
    end if
