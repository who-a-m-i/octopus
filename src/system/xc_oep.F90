...
contains
  ...
  subroutine xc_oep_init(oep, namespace, family, gr, st)
    ...
    !%End
    call message_g%obsolete_variable(namespace, 'OEP_Level', 'OEPLevel')
    call parse_variable(namespace, 'OEPLevel', XC_OEP_KLI, oep%level)
    if(.not. varinfo_valid_option('OEPLevel', oep%level)) call message_g%input_error('OEPLevel')

    if(oep%level /= XC_OEP_NONE) then
      if(oep%level == XC_OEP_FULL) then

        if(st%d%nspin == SPINORS) &
          call message_g%not_implemented("Full OEP with spinors")

        call message_g%experimental("Full OEP")    
        !%Variable OEPMixing
        !%Type float
        !%Default 1.0
        !%Section Hamiltonian::XC
        ...
        !%End
        call message_g%obsolete_variable(namespace, 'OEP_Mixing', 'OEPMixing')
     ...
     ! this routine is only prepared for finite systems. (Why not?)
      if(st%d%nik > st%d%ispin) &
        call message_g%not_implemented("OEP for periodic systems")
      ...
      if(st%d%nspin == SPINORS) &
        call message_g%experimental("OEP with spinors")

      if(st%d%kpt%parallel) &
        call message_g%not_implemented("OEP parallel in spin/k-points")
  ...
  subroutine xc_oep_write_info(oep, iunit)
    ...
    call message_g%print_var_option(iunit, 'OEPLevel', oep%level)
