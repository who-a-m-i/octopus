...
subroutine X(xc_oep_solve) (gr, hm, psolver, st, is, vxc, oep)
  ...
  do iter = 1, oep%scftol%max_iter
    ...
    case (OEP_MIXING_SCHEME_BB)
      ...
      if(debug%info) then
        write(messages(1), '(a,es14.6,a,es14.8)') "Info: oep%mixing:", oep%mixing, " norm2ss: ", dmf_nrm2(gr%mesh, ss)
  ...
  if(ff > oep%scftol%conv_abs_dens) then
    write(messages(1), '(a)') "OEP did not converge."
  ...
  write(messages(1), '(a,i4,a,es14.6)') "Info: After ", iter, " iterations, the OEP residual = ", ff
  messages(2) = ''
