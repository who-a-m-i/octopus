...
subroutine pes_mask_output_states(st, gr, geo, dir, outp, mask)
  ...
  do ik = st%d%kpt%start, st%d%kpt%end
    ...
    call batch_init(psib, st%d%dim, st%st_start, st%st_end, PsiAB(:, :, st%st_start:, ik), message_g)
...
subroutine pes_mask_interpolator_init(pesK, Lk, ll, dim, cube_f, interp, pmesh)
  ...
  if (dim  <  2 .or. dim > 3) then
    messages(1) = "This interpolator works only for 2 <= dim <= 3." 
...
subroutine pes_mask_output_full_mapM(pesK, file, namespace, Lk, ll, how, sb, pmesh)
  ...
  call cube_init(cube, ll, sb, namespace, message_g)
  call cube_function_null(cf)
  ...
  if(bitand(how, OPTION__OUTPUTFORMAT__NETCDF) /= 0) then
    ...
    write(messages(1), '(a)') 'Writing netcdf format file: '
  ...
  if(bitand(how, OPTION__OUTPUTFORMAT__VTK) /= 0)  then
    ...
    write(messages(1), '(a)') 'Writing vtk format file: '
    call message_g%info(1)
...
!   else
!     write(messages(1), '(a)') 'Writing ASCII format file: '
!     call message_g%info(1)
!     call out_ascii()
  end if
...
subroutine pes_mask_output_ar_polar_M(pesK, file, namespace, Lk, ll, dim, dir, Emax, Estep)
  ...
  if (  (dim  ==  1) ) then 
    messages(1)="Impossible to obtain angle-dependent quantities in 1D."
    call message_g%fatal(1)
...
subroutine pes_mask_dump(restart, mask, st, ierr)
  ...
  if (debug%info) then
    messages(1) = "Debug: Writing PES mask restart."
    call message_g%info(1)
  end if
    ...
    do ist = st%st_start, st%st_end
      do idim = 1, st%d%dim
        ...
        if (mpi_grp_is_root(mask%cube%mpi_grp) .and. mpi_grp_is_root(mask%mesh%mpi_grp)) then 
          ...
          if (err /= 0) then
            err2 = err2 + 1
            messages(1) = "Unable to write PES mask restart data to '"//trim(path)//"'."
  ...
  if (debug%info) then
    messages(1) = "Debug: Writing PES mask restart done."
    call message_g%info(1)
  end if
...
subroutine pes_mask_load(restart, mask, st, ierr)
  ...
  if (debug%info) then
    messages(1) = "Debug: Reading PES mask restart."
    call message_g%info(1)
  end if
  ...
  if (ierr == 0) then
    if (rr(1) /= mask%mask_r(1) .or. rr(2) /= mask%mask_r(2)) then
      messages(1) = "PhotoElectronSpectrum = pes_mask : The mask parameters have changed."
      messages(2) = "I will restart mapping from the previous context."
  ...
  if (debug%info) then
    messages(1) = "Debug: Reading PES mask restart done."
    call message_g%info(1)
  end if
