...
contains
  ...
  subroutine pes_spm_init(this, namespace, mesh, st, save_iter)
    ...
    !% </tt>
    !%End
    call message_g%obsolete_variable(namespace, 'PhotoElectronSpectrumPoints', 'PES_spm_points')
    this%sphgrid = .false.
    ...
    if(.not.varinfo_valid_option('PES_spm_recipe', this%recipe, is_flag = .true.)) &
      call message_g%input_error('PES_spm_recipe')
    call message_g%print_var_option(stdout, "PES_spm_recipe", this%recipe)
    ...
    if(this%onfly) then
      if(this%delomega <= M_ZERO) call message_g%input_error('PES_spm_DeltaOmega')
      call message_g%print_var_value(stdout, "PES_spm_DeltaOmega", this%delomega)
    end if
    ...
    if(this%sphgrid .and. this%nstepsthetar < 0) call message_g%input_error('PES_spm_StepsThetaR')
    ...
    if(this%sphgrid) then
      if(this%nstepsphir < 0)  call message_g%input_error('PES_spm_StepsPhiR')
    ...
    if(this%sphgrid) then
      if(parse_is_defined(namespace, 'PES_spm_Radius')) then
        ...
        if(radius <= M_ZERO) call message_g%input_error('PES_spm_Radius')
        call message_g%print_var_value(stdout, "PES_spm_Radius", radius)
      else
    ...
    if(.not. this%sphgrid) then
      ...
      if(mdim == 3) call message_g%print_var_value(stdout, "PES_spm_StepsThetaR", this%nstepsthetar)
      call message_g%print_var_value(stdout, "PES_spm_StepsPhiR", this%nstepsphir)

    end if

    call message_g%print_var_value(stdout, "Number of sample points", this%nspoints)
