...
contains
  ...
  ! ---------------------------------------------------------
  subroutine propagation_ops_elec_exp_apply(te, st, gr, psolver, dt)
    ...
    do ik = st%d%kpt%start, st%d%kpt%end
      do ib = st%group%block_start, st%group%block_end
        call exponential_elec_apply_batch(te, gr%der, psolver, st%group%psib(ib, ik), ik, dt)
      end do
    end do
  ...
  ! ---------------------------------------------------------
  subroutine propagation_ops_elec_fuse_density_exp_apply(te, st, gr, psolver, dt, dt2)
    type(exponential_elec_t), intent(inout) :: te
    type(states_elec_t),      intent(inout) :: st
    type(grid_t),             intent(inout) :: gr
    type(poisson_t),          intent(in)    :: psolver
