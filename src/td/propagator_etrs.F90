...
contains
  ...
  subroutine td_etrs(ks, parser, hm, gr, st, tr, time, dt, ionic_scale, ions, geo, move_ions)
    ...
    if(hm%theory_level /= INDEPENDENT_PARTICLES) then
      ...
      call propagation_ops_elec_fuse_density_exp_apply(tr%te, st, gr, CNST(0.5)*dt, dt)
      ...
      call propagation_ops_elec_exp_apply(tr%te, st, gr, CNST(0.5)*dt)
    ...
    call propagation_ops_elec_move_ions(tr%propagation_ops_elec, gr, hm, st, parser, ions, geo, &
    ...
    call propagation_ops_elec_update_hamiltonian(st, gr, hm, time)
    
    ! propagate dt/2 with H(time - dt)
    call propagation_ops_elec_fuse_density_exp_apply(tr%te, st, gr, M_HALF*dt)
  ...
  subroutine td_etrs_sc(ks, parser, hm, gr, st, tr, time, dt, ionic_scale, ions, geo, move_ions, sctol, scsteps)
    ...
    call propagation_ops_elec_fuse_density_exp_apply(tr%te, st, gr, M_HALF*dt, dt)
    ...
    call propagation_ops_elec_update_hamiltonian(st, gr, hm, time - dt)
  ...
  subroutine td_aetrs(parser, hm, gr, st, tr, time, dt, ionic_scale, ions, geo, move_ions)
    ...
    call propagation_ops_elec_exp_apply(tr%te, st, gr, M_HALF*dt)

    !Get the potentials from the interpolator
    call propagation_ops_elec_interpolate_get(gr, hm, tr%vksold)

    ! move the ions to time t
    call propagation_ops_elec_move_ions(tr%propagation_ops_elec, gr, hm, st, parser, ions, &
