...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine td_runge_kutta4(ks, parser, hm, gr, st, tr, time, dt, ions, geo)
    ...
    do
      ...
      call hamiltonian_elec_update(hm, gr%mesh, time = time - dt + c(1)*dt)
      call lda_u_update_occ_matrices(hm%lda_u, gr%mesh, st, hm%hm_base, hm%energy)
      vhxc1_op = hm%vhxc
      t_op  = time - dt + c(1) * dt
      rhs1 = M_z0
      ...
      call hamiltonian_elec_update(hm, gr%mesh, time = time - dt + c(2)*dt)
      call lda_u_update_occ_matrices(hm%lda_u, gr%mesh, st, hm%hm_base, hm%energy)
      vhxc2_op = hm%vhxc
      t_op  = time - dt + c(2) * dt
      rhs2 = M_z0
  ...
  subroutine td_rk4op(xre, xim, yre, yim)
    ...
    call hamiltonian_elec_update(hm_p, grid_p%mesh, time = t_op + c(1)*dt_op)
    ...
    call hamiltonian_elec_update(hm_p, grid_p%mesh, time = t_op + c(2)*dt_op)
    call lda_u_update_occ_matrices(hm_p%lda_u, grid_p%mesh, st_p, hm_p%hm_base, hm_p%energy)
    j = 1
    k = np * (kp2 - kp1 + 1) * (st2 - st1 + 1) * dim + 1
    do ik = kp1, kp2
  ...
  subroutine td_rk2op(xre, xim, yre, yim)
    ...
    hm_p%vhxc = vhxc1_op
    if(move_ions_op) hm_p%ep%vpsl = vpsl1_op
    call hamiltonian_elec_update(hm_p, grid_p%mesh, time = t_op + dt_op)
    call lda_u_update_occ_matrices(hm_p%lda_u, grid_p%mesh, st_p, hm_p%hm_base, hm_p%energy)
