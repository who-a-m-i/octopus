...
contains
  ...
  subroutine td_write_init(writ, parser, outp, gr, st, hm, geo, ks, ions_move, with_gauge_field, kick, iter, max_iter, dt, mc)
    ...
    if(writ%out(OUT_PROJ)%write .or. writ%out(OUT_POPULATIONS)%write &
      ...
      call states_elec_load(restart_gs, parser, writ%gs_st, gr, ierr, label = ': gs for TDOutput')
  ...
  subroutine td_write_angular(out_angular, parser, gr, geo, hm, st, kick, iter)
    ...
    do idir = 1, 3
       ...
       angular(idir) = M_TWO*real(zpert_states_elec_expectation_value(angular_momentum, parser, gr, geo, hm, st), REAL_PRECISION)
    end do
    ...
    subroutine dipole_matrix_elements(dir)
      ...
      do ik = 1, st%d%nik
        do ist = gs_st%st_start, st%nst
          ...
          do uist = gs_st%st_start, gs_st%st_end
            call states_elec_get_state(gs_st, gr%mesh, ist, ik, gspsi)
