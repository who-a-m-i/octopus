...
program dielectric_function
  ...
  if(parse_block(default_namespace, 'GaugeVectorField', blk) == 0) then
    ...
    if(in_file < 0) then 
      messages_lines(1) = "Cannot find the GaugeVectorField in the input file"
      call messages_fatal(1)
    end if

  end if

  messages_lines(1) = "This program assumes that the gauge field is in the 'x'"
  messages_lines(2) = "direction, and that the 'y' and 'z' directions are equivalent."
  messages_lines(3) = "If this is not the case the dielectric function and the"
  messages_lines(4) = "susceptibility will be wrong."
  ...
  if(in_file < 0) then 
    messages_lines(1) = "Cannot open file '"//trim(io_workpath('td.general/gauge_field', default_namespace))//"'"
    call messages_fatal(1)
  end if
  ...
  if(parse_is_defined(default_namespace, 'TransientAbsorptionReference')) then
    ...
    if(ref_file < 0) then
      messages_lines(1) = "Cannot open reference file '"// &
    ...
    if(time_steps_ref < time_steps) then
      messages_lines(1) = "The reference calculation does not contain enought time steps"
      call messages_fatal(1)
    end if
 
    if(dt_ref /= dt) then
      messages_lines(1) = "The time step of the reference calculation is different from the current calculation"
      call messages_fatal(1)
    end if
  ...
  write(messages_lines(1), '(a, i7, a)') "Info: Read ", time_steps, " steps from file '"// &
