...
program oct_floquet
  ...
  call hamiltonian_elec_epot_generate(sys%hm, sys%parser, gr, sys%geo, st, time=M_ZERO)
  call hamiltonian_elec_update(sys%hm, gr%mesh, time = M_ZERO)
...
contains
  ...
  subroutine floquet_solve_non_interacting()	
    ...
    do it=1,nT
      ...
      call hamiltonian_elec_update(sys%hm, gr%mesh, time=Tcycle+it*dt)
      ! get hpsi
