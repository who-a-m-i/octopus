...
contains
  ...
  subroutine read_resonances_file(order, ffile, namespace, search_interval, final_time, nfrequencies)
    ...
    if(order /= 2) then
      write(messages(1),'(a)') 'The run mode #3 is only compatible with the analysis of the'
      write(messages(2),'(a)') 'second-order response.'
    ...
    if(.not.file_exists) then
      write(messages(1),'(a)') "Could not find 'ot' file."
    ...
    if(iunit == 0) then
      write(messages(1),'(a)') 'Could not open '//trim(ffile)//' file.'
      call message_g%fatal(1)
    end if
    ...
    if(order_in_file /= order) then
      write(messages(1), '(a)') 'The ot file should contain the second-order response in this run mode.'
      call message_g%fatal(1)
    end if
    ...
    if(final_time > M_ZERO) then
      ...
      if(total_time > dt*time_steps) then
        ...
        write(messages(1), '(a)')        'The requested total time to process is larger than the time available in the input file.'
        write(messages(2), '(a,f8.4,a)') 'The time has been adjusted to ', &
  ...
  subroutine analyze_signal(order, omega, search_interval, final_time, nresonances, nfrequencies, &
    ...
    if(order_in_file /= order) then
      write(messages(1), '(a)') 'Internal error in analyze_signal'
      call message_g%fatal(1)
    end if
  ...
  subroutine find_resonance(omega, leftbound, rightbound, nfrequencies)
    ...
    if(ierr /= 0) then
      write(messages(1),'(a)') 'Could not find a maximum.'
      write(messages(2),'(a)')
      write(messages(3), '(a,f12.8,a,f12.8,a)') '   Search interval = [', &
        units_from_atomic(units%energy, leftbound), ',', units_from_atomic(units%energy, rightbound), ']'
      write(messages(4), '(a,f12.4,a)')         '   Search discretization = ', &
  ...
  subroutine resonance_first_order(omega, power, nw_subtracted, dw, leftbound, rightbound)
    ...
    write(messages(1), '(a)')                 '******************************************************************'
    write(messages(2), '(a,i3)')              'Resonance #', nw_subtracted + 1
    write(messages(3), '(a,f12.8,a,f12.8,a)') 'omega    = ', units_from_atomic(units_out%energy, omega), &
                                             ' '//trim(units_abbrev(units_out%energy))//' = ',  omega, ' Ha'
    write(messages(4), '(a,f12.8,a,f12.8,a)') 'C(omega) = ', units_from_atomic(units_out%length**2, power), &
                                             ' '//trim(units_abbrev(units_out%length**2))//' =', power, ' b^2'
    write(messages(5), '(a,f12.8,a,f12.8,a)') '<0|P|I>  = ', units_from_atomic(units_out%length, sqrt(abs(power))), &
                                             ' '//trim(units_abbrev(units_out%length))//' = ', sqrt(abs(power)),' b'
    write(messages(6), '(a,f12.8)')           'f[O->I]  = ', M_TWO*omega*power
    write(messages(7), '(a)')
    write(messages(8), '(a,f12.8,a,f12.8,a)') '   Search interval = [', &
      units_from_atomic(units_out%energy, leftbound), ',', &
      units_from_atomic(units_out%energy, rightbound), ']'
    write(messages(9), '(a,f12.4,a)')         '   Search discretization = ', units_from_atomic(units_out%energy, dw), &
  ...
  subroutine resonance_second_order(omega, power, nw_subtracted, leftbound, rightbound, c01, c02)
    ...
    write(messages(3), '(a,f12.8,a,f12.8,a)') 'omega    = ', units_from_atomic(units_out%energy, omega), &
                                             ' '//trim(units_abbrev(units_out%energy))//' = ', omega, ' Ha'
    write(messages(4), '(a,f12.8,a,f12.8,a)') 'C(omega) = ', units_from_atomic(units_out%length**3, power), &
                                             ' '//trim(units_abbrev(units_out%length**3))//' = ', power, ' b^3'
    call message_g%info(4)

    if(c01*c02 /= M_ZERO) then
      write(messages(1), '(a,f12.8)')         '    C(omega)/(C0i*C0j) = ', power / (c01 * c02)
      call message_g%info(1)
   end if
  
    write(messages(1), '(a)')
    write(messages(2), '(a,f12.8,a,f12.8,a)') '   Search interval = [', &
      units_from_atomic(units_out%energy, leftbound), ',', &
      units_from_atomic(units_out%energy, rightbound), ']'
    write(messages(3), '(a)')                 '******************************************************************'
    write(messages(4), '(a)')
  ...
  subroutine generate_signal(namespace, order, observable)
    ...
    if(nfiles == 0) then
      write(messages(1),'(a)') 'No multipoles.x file was found'
      call message_g%fatal(1)
    end if
    if(order > nfiles) then
      write(messages(1),'(a)') 'The order that you ask for is higher than the number'
      write(messages(2),'(a)') 'of multipoles.x file that you supply.'
      call message_g%fatal(2)
    end if
  ...
  subroutine read_ot(namespace, nspin, order, nw_subtracted)
    ...
    if(iunit == 0) then
      write(messages(1),'(a)') 'A file called ot should be present and was not found.'
      call message_g%fatal(1)
    end if
    ...
    elseif(index(line, '# (l, m) = ') /= 0) then
      ...
      write(messages(1),'(a)') 'Problem reading "ot" file: could not figure out the shape'
      write(messages(2),'(a)') 'of the observation operator.'
      call message_g%fatal(2)
    end if
...
program oscillator_strength
  ...
  if(ierr /= 0) then
    ...
    messages(2) = "the oct-oscillator-strength command is not available."
    call message_g%fatal(2)
  end if
